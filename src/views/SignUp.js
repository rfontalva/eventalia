...
const SignUp = () => {
  ...
  const submitHandler = async (e) => {
    e.preventDefault();
    const {
      firstname, lastname, username, email, password, password2, cbu
    } = account;
    ...
    if (!validate()) return;
    try {
      const res = await fetch(`/api/user?firstname=${firstname}&lastname=${lastname}&email=${email}&username=${username}&password=${password}&cbu=${cbu}`,
        { method: 'PUT' });
      switch (res.status) {
        case 401:
    ...
    <article className="grid">
      <div className="inputs-box">
        ...
        <form onSubmit={submitHandler}>
          ...
          <label htmlFor="cbu">
            CBU
            <input type="text" id="cbu" name="cbu" onChange={changeHandler} value={account.cbu} />
          </label>
